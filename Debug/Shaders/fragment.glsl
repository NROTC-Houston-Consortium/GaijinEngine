...
vec3 calculateDiffuse(Material material, vec3 vs_position, vec3 vs_noraml, vec3 lightPos0)
{
...
void main()
{
	...
	vec3 specularFinal = calculateSpecular(material,vs_position,vs_normal,lightPos0,cameraPos);
		...
		//vec4(vs_color, 1.f) *
	//fs_color =  texture2D(material.diffuseTex, vs_texcoord)  /* vec4(vs_color, 1.f) */ * (vec4(ambientFinal, 1.f) + vec4(diffuseFinal, 1.0f) + vec4(specularFinal,1.f));
		//fs_color = texture2D(texture0, vs_texcoord) * texture2D(texture1, vs_texcoord);
